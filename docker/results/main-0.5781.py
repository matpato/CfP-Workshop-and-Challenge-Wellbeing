...
def discretizeHandCraftedDataset(processedHandTrainDataset):
    ...
    for index_n_entries in range(n_entries):
        dataset_features[index_n_entries] = [
            ...
            ######## GSR Features
            np.nanquantile([i[5] for i in processedHandTrainDataset.gsrFeatures[index_n_entries]], q=0.85) - np.nanquantile([i[0] for i in processedHandTrainDataset.gsrFeatures[index_n_entries]], q=0.15),  # -0.009, covariance :-0.000
...
#clf = RandomForestClassifier(min_samples_leaf=10,max_depth=10, max_features=None) #BEST CLASSIFIER SO FAR - 0.45
clf = RandomForestClassifier(min_samples_leaf=10,max_depth=10,min_samples_split=6, max_features=None) #BEST CLASSIFIER SO FAR - 0.45
#clf = KNeighborsClassifier(algorithm='kd_tree',n_neighbors=20) # 0.53

#clf = LogisticRegression() #BEST CLASSIFIER SO FAR
